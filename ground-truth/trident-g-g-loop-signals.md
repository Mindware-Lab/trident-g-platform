...
For practical Trident-G use, the loop can be governed by a compact set of operational signals that sit above the deeper computational variables. These signals are not separate from the theory. They are the coaching-facing control board derived from the underlying quantities **F, G, F★, x, ξ, ε**, together with the **explore–exploit tilt δ**, the salience–valuation machinery, and map-cost considerations. The recommended signal set is:
...
The key distinction is that some of these are core state or error signals, while others are routing, budgeting, or map-management signals.

* **Load** is the main practical face of realised allostatic burden.
* **Mismatch** is the main practical face of allostatic prediction error.
* **State drift** is a practical indicator of deteriorating control and noisy credit assignment.
* **Unexpected mismatch** is the escalation signal for deeper restructuring.
* **Salience** sets rigour.
* **Complexity** belongs to the map dial.
* **Conflict** is best treated as an important cue source rather than a master scalar in its own right.

A further theoretical clarification is important. Mismatch is not a simple one-step switch between explore and exploit. Acute worse-than-expected reality typically produces short-horizon tightening and damage-control first. Persistent mismatch, repeated policy failure, or failed re-entry builds pressure for exploratory restructuring. Easier-than-expected reality relaxes unnecessary bracing, supports consolidation of the current policy, and may reopen safer exploration relative to the previous stance. In that sense, reinforcement learning is central, but it is embedded inside a broader Trident-G architecture of allostasis, salience, valuation, and portability testing.  
...
This is consistent with the wider architecture, which treats subjectivity as the lived interface of the loop rather than as either pure truth or pure noise. 
...
Load maps most closely to **F(t)**, the realised allostatic or free-energy burden. When expressed relative to the defended corridor, it can also be read through **x(t) = F(t) − F★(t)**, that is, current deviation from the corridor centre. So load is primarily the practical face of **F**, and secondarily of **x** when the question is burden relative to the defended band.  

### Practical meaning

How hard the situation is on the organism or learner right now, given complexity, time pressure, distraction, emotional strain, motivational conflict, uncertainty, and bodily cost. In user-facing G-Loop language, load is the strain level or mental effort cost.
...
Decay of the current task-set, control state, or bound relational structure.

### Computational status

State drift is best treated as an operational telemetry signal, not as one of the original primitive scalars in the Dec 2025 computational paper. It is a practical marker that the system is losing hold of the active set, with slipping attention, blurred rules, collapsing bindings, creeping errors, or degraded stability.

### Practical meaning

Wandering, loss of thread, state drift, rule-blur, unstable performance, or rising error bursts. State drift usually means that even if the map is notionally good, the system is no longer holding, binding, or executing it cleanly enough for trustworthy updating.

### Subjective face

State drift is often lived as:
...
* mental slipperiness
* loss of relational grip or collapse of who-goes-with-what in the current problem space
* shallow rereading without integration
* feeling cognitively smeared or scattered

It may also be accompanied by frustration or shame, which can worsen the drift if they are misread as reasons to push harder.

### Primary use in the loop

State drift is mainly a credit-assignment warning signal. High drift means that learning conditions are getting noisy. The default response is usually to reduce model weight, narrow the loop, stabilise, recover, or shorten the action cycle. In other words, drift is a strong cue to simplify before interpreting too much. This fits the wider framework’s use of drift as a feedback signal for clean execution rather than as a deep computational primitive. 
...
Mismatch maps directly to **ε(t) = F(t) − G(t) = x(t) − ξ(t)**. This is the cleanest one-to-one mapping in the framework. It captures whether the niche is harsher than expected or easier than expected. 

### Practical meaning

Plan–reality gap, prediction miss, or the sense that the current model, policy, or bracing level is off. In applied G-Loop wording, mismatch means **“something doesn’t add up”** because realised conditions differ from expected burden.
...
This is consistent with the computational theory’s treatment of ε as the miscalibration signal that drives learning, engagement shifts, and mode switching, while δ is shaped by value differences over exploitative versus exploratory policies.  
...
These feelings can be extremely important because they often mark first contact with model error. But they do not yet tell you whether the right response is simplification, probing, or deeper restructuring.
...
Unexpected mismatch is best treated as a later Trident-G refinement: **mismatch prediction error**, that is, mismatch relative to what mismatch was expected in that context. It is not one of the deepest primitive scalars in the Dec 2025 paper, but it is a valuable higher-order applied signal for routing. 
...
Unexpected mismatch is the escalation signal for deeper revision. In the later theory and protocol, high unexpected mismatch, especially when paired with re-entry failure, is what increases the probability that the system should move from ordinary tuning into structural revision. In practice, this means: stop merely tuning execution and consider revising the schema, boundary, trigger, tactic, strategy, or state abstraction itself. 
...
Salience corresponds to the theory’s **salience–allostasis hub**, which pools internal and external signals and helps determine valuation, σ-fields, and the value difference that shapes explore–exploit tilt. In later wording, salience is a precision-weighted integration of interoceptive and exteroceptive demand signals. 
...
The representational burden or model weight of the current map, including the cost of holding temporary, arbitrary, novel relational bindings online.
...
How many moving parts the model is tracking, how costly it is to hold and bind online, and how fragile it becomes under load and state drift.
...
* confusion without a clear crux
...
Conflict is best treated as a cue source, not as a master scalar on the same level as load or mismatch. In the wider theory, conflict is one of the demand or uncertainty cues that can feed salience and bracing.
...
* affective capture may worsen state drift
...
### 1. Check load and state drift first
...
* **State drift:** how much the task-set or bound structure is decaying.
...
* **Complexity:** how heavy the current map is to hold, bind, and run.
* **Conflict:** where local incompatibility or competition requires sharper checking.

## C10. Summary statement

Load and mismatch are the main practical faces of the underlying Trident state variables. State drift tracks control decay. Unexpected mismatch is the restructuring gate. Salience sets rigour. Complexity governs model weight and the burden of holding bound relational structure online. Conflict is a cue source that routes the loop towards evaluation or discrimination.
