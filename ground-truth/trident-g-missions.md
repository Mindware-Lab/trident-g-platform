...
Trident-G uses a single G-Loop kernel, but it is helpful to distinguish four applied layers rather than treating everything as one flat set of “missions”.
...
A further clarification from the core G-Loop is important here. The shared kernel not only keeps cognition viable and supports good inference. It also progressively extracts gist, selects deeper invariants, and compiles what survives into reusable Gc. In that sense, the mission families below should be read as different applied purposes for running one common inference-and-compression loop, not as separate theories.

This appendix presents the applied four-layer distinction used for programme design. It sits within the wider Trident-G architecture, which also includes the language of thought and action, reasoning calculus, semantic layer, syntax layer, subjective interface, Ψ-allostatic geometry, and cognitive runtime.

Above the applied four-layer distinction sit two highest-level meta-missions, **Viability** and **Expansion**. These should be treated not only as architectural statements, but as trainable global orientations: the system is learning to prefer stable, recoverable in-band functioning and the enlargement of viable dynamic range and portable competence, while the meta-dials define the slower governance qualities by which those aims are realised.

Higher-level note on Trident-G itself: the mission framework below describes the applied purposes for which the G-Loop is run. At the deeper theoretical level, Trident-G is organised as a shaft-and-tines architecture in which the shaft is banked Gc competence, the centre tine is live Gf reasoning, and the outer tines are the dominant explore–exploit branches available under higher engagement. This architecture is governed by two meta-missions: **Viability**, the maintenance and recovery of clean operation within the defended Ψ corridor, and **Expansion**, the enlargement of viable dynamic range and portable operator repertoire through the coordination of exploration, exploitation, gist extraction, and crystallisation into reusable Gc. The mission families below are therefore best understood as high-level purpose frames that sit on top of this common Trident-G architecture.

## B1. Mission frames — IQ / Performance

Mission frames are the purpose frames of the inference engine. They define what counts as a good map, a good test, a good update, and a good stopping point for the current run. The same language of thought and action can be used in every frame, but the success criterion changes by frame.

A good mission-frame map is not only predictive structure. It also includes a value landscape: what matters, what is costly, what is worth checking, and what counts as success under the present stakes.
...
**Typical operator emphasis:** Y1 Orient, Y4 Map, Y5 Chunk, Y6 Unpack, Y8 Abstract, Y10 Re-represent, Y11 Meta-map, Y12 Evaluate, Y13 Discriminate / Probe.
...
**Typical operator emphasis:** Y1 Orient, Y4 Map, Y7 Analogise, Y9 Instantiate, Y10 Re-represent, Y12 Evaluate, Y13 Discriminate / Probe, Y15 Satisfice / Commit.
...
**Typical operator emphasis:** Y1 Orient, Y4 Map, Y7 Analogise, Y9 Instantiate, Y12 Evaluate, Y13 Discriminate / Probe, Y15 Satisfice / Commit, Y16 Level control.
...
**Typical operator emphasis:** Y1 Orient, Y4 Map, Y9 Instantiate, Y12 Evaluate, Y13 Discriminate / Probe, Y15 Satisfice / Commit, Y16 Level control, with EOU as a composition pattern and Y2 Recover or Y3 Externalise / Escalate when viability is threatened.

### B1.5 Learn | Acquisition frame

**Purpose:** acquire, stabilise, and transfer a new capability, method, or schema.  
**Primary success criterion:** the learner can instantiate the skill or schema in a new syntax or wrapper, explain the cue, and show early portability.
...
**Operator anchors:** Y2 Recover, and Y3 Externalise / Escalate when safe restoration is not possible.

### B2.8 Externalise / escalate
...
## B3. Meta-dials and slow shapers of cognitive geometry

This layer should be kept conceptually distinct from both the fast loop and the two highest-level meta-missions. These are not best understood as in-session missions. They are slower governance variables that shape controllability, learnability, corridor width, ecological support, proof discipline, compression depth, scale fit, and noise over longer timescales. In other words, they change the geometry of cognition rather than merely the content of one run.
...
The theory also includes slower offsets such as allostatic drift, agency or gearing bias, and risk tilt. These are not “missions” but longer-timescale tendencies that bias where the system starts, how readily it gears for challenge, and whether uncertainty evokes widening or locking-in by default.
...
In practice, these signals are often first encountered through the subjective interface of the loop: feelings, urges, motivations, emotional pressure, and felt significance. Coaching should therefore treat subjectivity as signal-rich but fallible. It can reveal real load, salience, mismatch, or conflict, but it can also reflect over-bracing, fatigue, stale priors, or distorted appraisal.

### B4.1 Load

**Meaning:** current realised demand, strain, or effort cost.  
**Typical sources:** complexity, time pressure, emotion, motivational conflict, distraction, uncertainty.  
...
### B4.8 Practical note on complexity, effort, and challenge

Two useful clarifications follow from the wider G-Loop.

**Revision cost** is the cost of changing the current model, policy, or skill structure to fit what is happening. This is felt mainly through mismatch and unexpected mismatch, which drive updating versus deeper restructuring.

**Map cost** is the cost of carrying and running the current model. This is what “complexity” refers to in the present applied signal layer.

At the lived level, these interact with perceived effort and current challenge state. Perceived effort is the felt estimate of the total cost of running the loop under current stakes and state, including the costs of updating, holding the map, sustaining attention, and staying viable enough to continue. Current challenge state is the present runtime burden of keeping a usable model online for understanding or action.

A useful practical rule is that enjoyable challenge occurs when complexity is high enough to be engaging, but still low enough for the system to update, compress, and act without losing viable control.
...
Meta-dials optimise the longer-timescale conditions and governance quality of cognition: controllability, adherence, ecological fit, compression depth, proof discipline, scale fit, progress–quality balance, exploration–exploitation balance, noise reduction, learning rate, and stable opportunity for the loop to run well.
...
A script counts as banked only after it survives validation, typically via syntax swap, wrapper swap, stake swap, and boundary or trap checks, so the stored unit is portable rather than wrapper-bound.
...
What is banked is not the whole episode. It is the portable gist, invariant, or script that survives pressure and can later be re-instantiated in new contexts.

## B7. One-line summary

Mission frames define what the loop is for. Control missions keep the loop viable. Meta-dials shape the long-timescale geometry of cognition. Feedback signals tell the loop whether to simplify, probe, recover, or restructure. The shared kernel progressively extracts gist, selects deeper invariants, and compiles what survives into reusable Gc.
...
**Shaft = Gc**  
the banked, reusable support structure, the validated operator library, compressed gist, or crystallised competence

**Centre tine = Gf**  
the live relational bridge, effortful flexible reasoning in the centre of the loop
...
That is fully consistent with the core claim that the organism must stay within a defended viability band and with the G-Loop definition as the smallest repeatable cycle that both keeps cognition viable inside the Ψ-band and turns successful runs into reusable know-how.

**Meta-mission 2: Expansion/G Augmentation**  
...
These meta-missions should be treated as trainable global orientations, not as peer meta-dials. Viability is the cultivated preference for clean, recoverable in-band functioning. Expansion is the cultivated preference for enlarging live reasoning range and banked portable competence rather than merely securing local wrapper-bound success.
