...
import { postIqProEvent, queueIqProSync } from "./runtime/cloud/iq-pro-cloud.js";
...
function saveState(nextState = state) {
  ...
  queueIqProSync("capacity", "capacity_save");
  return state;
}
...
function saveActiveModule(moduleId) {
  ...
  queueIqProSync("activeModule", "module_switch");
...
function saveTrackerState(nextState = trackerState) {
  ...
  queueIqProSync("tracker", "tracker_save");
  return trackerState;
}
...
function saveReasoningState(nextState = reasoningState) {
  ...
  queueIqProSync("reasoning", "reasoning_save");
...
function saveUnifiedCoachState(nextState = coachState) {
  ...
  queueIqProSync("coach", "coach_save");
  return coachState;
}
...
function saveEconomy(nextEconomy = economy) {
  ...
  queueIqProSync("economy", "economy_save");
  return economy;
}
...
function nextCoachSessionDisplay() {
  const contract = currentCoachContract();
  const session = activeCoachSession();
  if (contract?.routeClass === "support" || session?.routeClass === "support") {
    return `Support ${clampSessionCounter(coachState?.programme?.supportSessionNumber) + 1}`;
  }
  const completed = completedProgrammeSessions();
...
function ensureZonePulseController() {
  ...
  zonePulseState.controller = createZoneProbeController({
    ...
    onComplete(summary) {
      ...
      queueIqProSync("zone", "zone_complete");
      postIqProEvent("zone_check_complete", {
        valid: summary.valid === true,
        state: summary.state,
        confidence: summary.confidence,
        bitsPerSecond: summary.bitsPerSecond ?? null
      });
...
async function finishReasoningBlock({ nextStep = "finish_session" } = {}) {
  ...
  postIqProEvent("reasoning_block_complete", {
    family: summary.family,
    mode: summary.mode,
    accuracy: summary.accuracy,
    transferScore: summary.transferScore?.total ?? null,
...
function finishBlock() {
  ...
  saveState();
  postIqProEvent("capacity_block_complete", {
    wrapper: entry.wrapper,
    routeClass: entry.routeClass,
    accuracy: entry.block?.accuracy ?? null,
    transferScore: entry.transferScore?.total ?? null,
    sessionComplete: Boolean(unifiedCompleted)
  });
  activeBlock = null;
...
function programmeSessionGroups() {
  ...
  const chronological = state.history
    ...
    .filter((entry) => entry && (manualMode
      ...
      : (entry.routeClass === "core" || entry.rewardMode === "core" || entry.routeClass === "support" || entry.rewardMode === "support")));

  chronological.forEach((entry) => {
    const key = entry.sessionId || entry.id || `block_${entry.tsStart || groups.length}`;
    const isSupport = !manualMode && (entry.routeClass === "support" || entry.rewardMode === "support");
    const numberedSession = manualMode ? entry.manualSessionNumber : (isSupport ? null : entry.coreSessionNumber);
    let group = byKey.get(key);
    if (!group) {
      group = {
        key,
        routeClass: isSupport ? "support" : (entry.routeClass || entry.rewardMode || null),
  ...
  return groups.map((group, index) => {
    const isSupport = group.routeClass === "support";
    if (isSupport) supportIndex += 1;
    ...
    return {
      ...
      label: isSupport ? `S${supportIndex}` : `${sessionNumber}`,
      avgN,
      avgTransfer,
...
function sessionStatsModel() {
  ...
  const supportSessions = manualMode ? 0 : groups.filter((group) => group.routeClass === "support").length;
  return {
    slots,
    rolling,
    manualMode,
    ...
    <section class="stats-graph" aria-label="${escapeHtml(title)}">
      ...
      <div class="stats-bars" style="--stats-bars-count:${Math.max(1, model.slots.length)};">
        ${model.slots.map((slot) => {
          ...
          const sessionLabel = slot.empty
            ? `Session ${slot.slot}`
            : slot.routeClass === "support" ? `Support route ${slot.label}` : `Session ${slot.label}`;
          return `
...
function renderCenterStatsDashboard() {
  ...
  const rangeLabel = model.manualMode
    ...
    : (model.rolling ? "Last 20 coach routes" : "20-session programme");
  const sessionSubtitle = model.manualMode
    ? "Mean held N across each manual session."
    : "Mean held N across coached core and support routes.";
  return `
    <div class="center-stats-dashboard">
      ...
      <div class="center-stats-summary">
        ...
        ${!model.manualMode && model.supportSessions ? `<div class="stat"><span class="mini-label">Support Routes</span><strong>${model.supportSessions}</strong></div>` : ""}
...
function saveCompletedTrackerSession() {
  ...
  saveTrackerState();
  postIqProEvent("test_result_saved", {
    testId: entry.testId,
    type: entry.type,
    result: entry.result
  });
